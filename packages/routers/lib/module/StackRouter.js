      ...
      switch (action.type) {
        ...
        case 'MOVE_TO_BOTTOM':
          {
            ...
            if (index > 0) {
              const routes = state.routes.slice(0, index).concat(state.routes.slice(index + 1));
              return _objectSpread({}, state, {
        ...
        case 'REUSE_AND_PUSH':
          if (action.payload.key || action.payload.name && state.routeNames.includes(action.payload.name)) {
            ...
            return _objectSpread({}, state, {
              index: state.routes.length - 1,
              routes: [...state.routes.slice(0, index), ...state.routes.slice(index + 1, state.routes.length), action.payload.params !== undefined ? _objectSpread({}, state.routes[index], {
